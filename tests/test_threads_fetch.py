...
class ParseThreadsHtmlTests(unittest.TestCase):
    ...
    def test_text_post_keeps_instagram_forwarded_image_meta(self):
        html = """
        <html><head>
          <meta property="og:description" content="轉發 IG 圖片貼文" />
          <meta property="og:image" content="https://scontent.cdninstagram.com/v/t51.2885-15/123456_1080x1350.jpg" />
          <meta property="og:image:width" content="1080" />
        ...
        post = _parse_html(
            "https://www.threads.net/@tester/post/IG123",
            html,
        )

        self.assertEqual(post.text, "轉發 IG 圖片貼文")
        self.assertEqual([media.type for media in post.media], ["image"])
        self.assertEqual(
            post.media[0].url,
            "https://scontent.cdninstagram.com/v/t51.2885-15/123456_1080x1350.jpg",
        )
